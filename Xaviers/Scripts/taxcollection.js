var url = '/api/TaxCollection';
var odaturl = '/odata/OdataTaxCollection?$orderby=ReceivedDate desc';
var typeaheadurl = "/api/Contacts";
var pdfUrl = "/odata/OdataTaxCollection?$orderby=ReceivedDate desc"
var typeadhedTax = '/odata/TaxOdata?$select=Id,TaxName&$orderby=StartDate desc'
function taxCollectionCtrl($scope, repository, $http, utilityService) {
        ...
        $scope.payerDetail != null && angular.isDefined($scope.payerDetail) && angular.isDefined($scope.payerDetail.ContactId))
        {
            ...
            var taxAmt = repository.getWebApi(url + '/' + taxids).then(function (taxAmount) {
                if (taxAmount != null && angular.isDefined(taxAmount) && taxAmount.length > 0) {
                    ...
                    if($scope.Tax == null || angular.isUndefined($scope.Tax))
                    {
                        $scope.Tax = {}
                    }
                    $scope.Tax.Amount = taxAmount[0].BalanceAmount;
                }
            });
        }
    }
        ...
        repository.get(function (results) {
            ...
            if (results.value != null && angular.isDefined(results.value) && angular.isObject(results.value) && results.value.length > 0) {
                angular.forEach(results.value, function (dta, index) {
                    if (taxids != '')
                    {
                        var tx = ',' + dta.ContactId + '|' + dta.TaxId;
                        taxids += tx;
                    }
                    else
                    {
    ...
    $scope.edit = function (id) {
        ...
        repository.get(function (results) {
            ...
            $scope.BalanceAmount = $scope.getBalance($scope.Tax.TaxId, $scope.Tax.ContactId);
            $scope.TotalReceivedAmount = $scope.getReceived($scope.Tax.TaxId, $scope.Tax.ContactId);
        }, editUrl);
        $scope.isUpdate = true;
        $scope.isList = false;
        $scope.isView = false;
    ...
    $scope.save = function (myForm, isApprove) {
        ...
        if ($scope.Tax == null || angular.isUndefined($scope.Tax.TaxId) || $scope.Tax.TaxId == '') { alert("Please select tax from dropdown"); return false; }
        $scope.isUpdate = false;
    ...
    $scope.changeTax = function () {
        ...
        if ($scope.Tax != null && angular.isDefined($scope.Tax) && angular.isDefined($scope.Tax.TaxId)) {
            delete $scope.Tax.TaxId;
        }
    }
        ...
        repository.getTypeAhead(odaturl + $scope.filterCondition).then(function (pdfTax) {
            ...
            if (pdfTax != null && !angular.isUndefined(pdfTax)) {
                angular.forEach(pdfTax, function (tax, index) {
                    ...
                    pdfTax.ReceivedTotal = ' ' + $scope.getReceived(tax.TaxId, tax.ContactId) + ' ';
                    pdfTax.Balance = ' ' + $scope.getBalance(tax.TaxId, tax.ContactId) + ' ';
    ...
    $scope.exportToPdfSingle = function (id) {
        ...
        repository.getTypeAhead(pdfUrl + '&$filter=(Id eq ' + id + ')').then(function (pdfTax) {
            if (pdfTax != null && !angular.isUndefined(pdfTax) && pdfTax.length > 0) {
                ...
                taxPdf.push({ Name: "Tax Name", Value: tax.TaxName != null && !angular.isUndefined(tax.TaxName) && tax.TaxName != '' ? tax.TaxName  + ' -  ' + tax.TaxId: ' ' });
                ...
                taxPdf.push({ Name: "Total Received", Value: ' ' + $scope.getReceived(tax.TaxId, tax.ContactId) + ' ' });
                taxPdf.push({ Name: "Balance", Value: ' ' + $scope.getBalance(tax.TaxId, tax.ContactId) + ' ' });
...
function setPayerDetails(scope) {
    ...
    if (scope.taxDetail != null && angular.isDefined(scope.taxDetail) && angular.isDefined(scope.taxDetail.Id) && scope.taxDetail.Id > 0) {
        ...
        scope.Tax.TaxId = scope.taxDetail.Id;
    }
    else {
        scope.Tax.ContactName = scope.payerDetail;
    }
...
function getPayerDetails(scope) {
    ...
    if (scope.Tax.TaxId != null && angular.isDefined(scope.Tax.TaxId) && scope.Tax.TaxId != "" && scope.Tax.TaxId > 0) {
        scope.taxDetail.TaxName = scope.Tax.TaxName;
        scope.taxDetail.Id = scope.Tax.TaxId;
    }
    else {
